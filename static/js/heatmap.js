...
let globalViolenceData = null;

function initHeatmap() {
  const container = document.getElementById("worldHeatmap");
  if (!container) return;

  // Clear existing SVG if any (for resize)
  d3.select("#worldHeatmap").selectAll("svg").remove();

  const rect = container.getBoundingClientRect();
  // Ensure we have some dimensions
  const width = Math.max(300, Math.round(rect.width || 750));
  const height = Math.max(240, Math.round(rect.height || 500));
  ...
  // Reset should return to the default (fitExtent) view
  ...
  const colorScale = d3
    .scaleLinear()
    .domain([0, 15, 30, 50])
    .range(["#d1d0d0", "#bb99ff", "#7303c0", "#0a015b"])
    .clamp(true);
  ...
  const violenceData = globalViolenceData;

  if (!topology || !violenceData) return;

  const countries = topojson.feature(topology, topology.objects.countries);

  // Filter out Antarctica (010) for better fitting
  const countriesForFitting = {
    type: "FeatureCollection",
    features: countries.features.filter((d) => d.id !== "010"),
  };

  // Auto-fit the projection to the SVG area.
  const fitPad = 10;
  projection.fitExtent(
    [
      [fitPad, fitPad],
      [width - fitPad, height - fitPad],
    ],
    countriesForFitting
  );
  ...
  const rawMaxValue = Math.max(
    ...Object.values(violenceData).map((d) => d.score)
  );
  const minValue = Math.min(...Object.values(violenceData).map((d) => d.score));
  ...
  g.selectAll(".country")
    ...
    .attr("fill", (d) => {
      ...
      if (countryName && violenceData[countryName]) {
        return colorScale(violenceData[countryName].score);
      }
      return "#e0e0e0";
    })
    .attr("stroke", "#ffffff")
    ...
    .on("mouseover", function (event, d) {
      d3.select(this).attr("stroke", "#1f2937").attr("stroke-width", 2).raise();
      const countryName = Object.keys(countryNameToId).find(
        (name) => countryNameToId[name] === d.id
      );
      if (countryName && violenceData[countryName]) {
  ...
  window.updateHeatmap = function (region, country) {
    ...
    if (country && country !== "All Countries") {
      ...
      fetch(`/api/countries?region=${encodeURIComponent(region)}`)
        ...
        .catch((err) =>
          console.error("Error fetching countries for region:", err)
        );
    } else {
      applyUpdate(null);
    }
...
Promise.all([
  ...
  .then(([topology, violenceData]) => {
    ...
    globalViolenceData = violenceData;
    initHeatmap();
  })
  .catch((error) => {
